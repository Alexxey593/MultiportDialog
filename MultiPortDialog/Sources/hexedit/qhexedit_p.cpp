...
// const int HEXCHARS_IN_LINE = BYTES_PER_LINE * 3 - 1; // 47
...
void QHexEditPrivate::append(const QByteArray& ba)
{
    if (ba.length() > 0) {
        if (_overwriteMode) {
            QUndoCommand* arrayCommand = new ArrayCommand(&_xData, ArrayCommand::replace, _xData.size(), ba, ba.length());
            _undoStack->push(arrayCommand);
            emit dataChanged();
        }
        else {
            QUndoCommand* arrayCommand = new ArrayCommand(&_xData, ArrayCommand::insert, _xData.size(), ba, ba.length());
            _undoStack->push(arrayCommand);
            emit dataChanged();
        }
        // _xData.insert(_xData.size(), ba);
        adjust();
...
void QHexEditPrivate::setCodecName(QByteArray name)
{
...
void QHexEditPrivate::setSelectByOneByte(bool mode)
{
    resetSelection();
    selectByOneByte = mode;
}
...
void QHexEditPrivate::setBytesPerLine(int bpl)
{
    ...
    BYTES_PER_LINE = bpl;
    HEXCHARS_IN_LINE = BYTES_PER_LINE * 3 - 1;
    adjust();
}

void QHexEditPrivate::addColorizedArea(int begin, int end, QColor color)
{
    colorizedAreas.append(ColorizedArea(begin, end, color));
    int len = end - begin;
    if (maxColorAreaLen < len)
        maxColorAreaLen = len;
    adjust();
}

void QHexEditPrivate::addColorizedArea(QByteArray data, QColor color)
{
    if (data.isEmpty())
        return;
    auto dataSet = qMakePair(data, color);
    highlighters.append(dataSet);
    if (maxColorAreaLen < data.size())
        maxColorAreaLen = data.size();
    adjust();
}

void QHexEditPrivate::resetColorizedAreas()
{
    colorizedAreas.clear();
    highlighters.clear();
    maxColorAreaLen = 0;
    adjust();
}
...
void QHexEditPrivate::keyPressEvent(QKeyEvent* event)
{
    ...
    if (event->matches(QKeySequence::MoveToNextChar)) {
        if (selectByOneByte) {
            if ((selectedByte >= 0) && (selectedByte < (_xData.size() - 1)))
                selectedByte++;
        }
        else {
    ...
    if (event->matches(QKeySequence::MoveToPreviousLine)) {
        if (selectByOneByte) {
            if (selectedByte > BYTES_PER_LINE)
                selectedByte -= BYTES_PER_LINE;
        }
        else {
    ...
    if (event->matches(QKeySequence::MoveToNextLine)) {
        if (selectByOneByte) {
            if ((selectedByte >= 0) && (selectedByte < _xData.size() - BYTES_PER_LINE))
                selectedByte += BYTES_PER_LINE;
        }
        else {
            setCursorPos(_cursorPosition + (2 * BYTES_PER_LINE));
...
void QHexEditPrivate::paintEvent(QPaintEvent* event)
{
    ...
    if (_addressArea)
        painter.fillRect(QRect(_xPosAdr, event->rect().top(), _xPosHex - GAP_ADR_HEX + 2, event->rect().height()), _addressAreaColor);
    if (_asciiArea) {
        int linePos = _xPosAscii - (GAP_HEX_ASCII / 2);
        painter.setPen(Qt::gray);
        painter.drawLine(linePos, event->rect().top(), linePos, /*height()*/ event->rect().bottom());
    }

    if (BYTES_PER_LINE > 8) {
        QPen pen(QBrush(Qt::gray), 1, Qt::DotLine);
        painter.setPen(pen);
        for (int i = 8; i < BYTES_PER_LINE; i += 8) {
            int linePos = _xPosAscii + _charWidth * i;
            painter.drawLine(linePos, event->rect().top(), linePos, event->rect().bottom());
            linePos = _xPosHex + _charWidth * 3 * i - _charWidth / 2;
            painter.drawLine(linePos, event->rect().top(), linePos, event->rect().bottom());
        }
    }
    ...
    // My
    int dataHeight = ((_xData.size() / BYTES_PER_LINE + 1) * _charHeight) + 5;
    int firstLineIdx = ((dataHeight - (height() - event->rect().top())) / _charHeight - _charHeight) * BYTES_PER_LINE;
    if (firstLineIdx < 0)
        firstLineIdx = 0;
    int lastLineIdx = ((dataHeight - (height() - event->rect().bottom())) / _charHeight + _charHeight) * BYTES_PER_LINE;
    if (lastLineIdx > _xData.size())
        lastLineIdx = _xData.size();
    int yPosStart = ((firstLineIdx) / BYTES_PER_LINE) * _charHeight + _charHeight - (dataHeight - minimumHeight());
    // endMy

    // paint address area
    if (_addressArea) {
        for (int lineIdx = firstLineIdx, yPos = yPosStart; lineIdx < lastLineIdx; lineIdx += BYTES_PER_LINE, yPos += _charHeight) {
            QString address = QString("%1").arg(lineIdx + _xData.addressOffset(), _xData.realAddressNumbers(), 16, QChar('0'));
    ...
    // calculating visible colorized areas
    QList<ColorizedArea> visibleColorizedAreas;
    if (!highlighters.isEmpty() && _xData.size() > 0) {
        int startDataIdx = firstLineIdx - maxColorAreaLen;
        if (startDataIdx < 0) {
            startDataIdx = 0;
        }
        int lastDataIdx = lastLineIdx + BYTES_PER_LINE + maxColorAreaLen;
        if (lastDataIdx > _xData.size())
            lastDataIdx = _xData.size();
        QByteArray visibleData = _xData.data().mid(startDataIdx, lastDataIdx - startDataIdx);
        for (int i = 0; i < highlighters.size(); i++) {
            int pos = -1;
            do {
                pos = visibleData.indexOf(highlighters.at(i).first, pos + 1);
                if (pos >= 0) {
                    int globalPos = startDataIdx + pos;
                    if (globalPos < 0)
                        globalPos = 0;
                    visibleColorizedAreas.append(ColorizedArea(globalPos, globalPos + highlighters.at(i).first.size() - 1, highlighters.at(i).second));
                }
            } while (pos >= 0);
        }
    }
    ...
    for (int lineIdx = firstLineIdx, yPos = yPosStart; lineIdx < lastLineIdx; lineIdx += BYTES_PER_LINE, yPos += _charHeight) {
        ...
        for (int colIdx = 0; ((lineIdx + colIdx) < _xData.size() && (colIdx < BYTES_PER_LINE)); colIdx++) {
            ...
            if ((getSelectionBegin() <= posBa) && (getSelectionEnd() > posBa) && !selectByOneByte) {
                painter.setBackground(selected);
                painter.setBackgroundMode(Qt::OpaqueMode);
                painter.setPen(colSelected);
            }
            else if (selectByOneByte && (posBa == selectedByte)) {
                painter.setBackground(selected);
                painter.setBackgroundMode(Qt::OpaqueMode);
                painter.setPen(colSelected);
            }
                ...
                if (_highlighting) {
                    ...
                    if (/*colorizedAreas*/visibleColorizedAreas.isEmpty()) {
                        painter.setPen(colStandard);
                        painter.setBackgroundMode(Qt::TransparentMode);
                    }
                    else {
                        bool wasColorized = false;
                        foreach (ColorizedArea area, /*colorizedAreas*/ visibleColorizedAreas) {
                            if ((posBa >= area.begin) && (posBa <= area.end)) {
                                painter.setBackground(QBrush(area.color));
                                painter.setBackgroundMode(Qt::OpaqueMode);
                                painter.setPen(colStandard);
                                wasColorized = true;
                                break;
            ...
            if (colIdx == 0) {
                ...
                if (selectByOneByte) {
                    ...
                    painter.drawText(xPos + _charWidth, yPos, hex);
                }
                else {
            ...
            if (_asciiArea) {
                char c = _xData.asciiChar(lineIdx + colIdx).toLatin1();
                QString str = codec->toUnicode(&c, 1);
...
int QHexEditPrivate::cursorPos(QPoint pos)
{
    ...
    if ((pos.x() >= _xPosHex) && (pos.x() < (_xPosHex + HEXCHARS_IN_LINE * _charWidth))) {
        ...
        if (selectByOneByte && (x < 0))
            y = -1;
        result = x + y;
    }
    else if ((pos.x() >= _xPosAscii) && (pos.x() < (_xPosAscii + BYTES_PER_LINE * _charWidth))) {
        int y = ((pos.y() - 3) / _charHeight) * 2 * BYTES_PER_LINE;
        int x = (pos.x() - _xPosAscii) / _charWidth;
        result = x * 2 + 1 + y;
    }
...
void QHexEditPrivate::adjust()
{
    ...
    int newHeight = ((_xData.size() / BYTES_PER_LINE + 1) * _charHeight) + 5;
